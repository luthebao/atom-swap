...
contract ERC20Mintable is Context, AccessControl, ERC20Burnable {
    ...
    function mint(address to, uint256 amount) public virtual {
        require(
            ...
            "ERC20Mintable: must have minter role to mint"
        );
        _mint(to, amount);
    }
...
abstract contract TransactionThrottler is Ownable {
    ...
    modifier transactionThrottler(
        ...
        if (_tradingStart > block.timestamp) {
            require(
                _isWhitelisted[sender] || _isWhitelisted[recipient],
                "Protection: Transfers disabled"
            );
        } else if (
            _tradingStart <= block.timestamp &&
            _restrictionEndTime > block.timestamp
        ) {
            ...
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(
                    _previousTx[sender] + 60 <= block.timestamp &&
                        _previousTx[recipient] + 60 <= block.timestamp,
...
contract Token is ERC20Mintable, TransactionThrottler {
    constructor() ERC20Mintable("TEST_USDC", "testUSDC") {
        _mint(msg.sender, 20000000000000e18);
    }
