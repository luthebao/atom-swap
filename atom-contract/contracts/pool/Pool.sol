 ...
 */
contract Pool is Ownable, ReentrancyGuard, Pausable, Core, IPool {
    ...
    function swap(
        ...
        toAsset.addLiability(_dividend(haircut, retentionRatio));
        toAsset.transferUnderlyingToken(to, actualToAmount);

        if (haircut > 0) {
            feeCollector.collectFees(fromAsset, haircut);
        }
    ...
    function noHaircutSwap(
        ...
        address to,
        uint256 deadline
    ) external override ensure(deadline) nonReentrant whenNotPaused onlyOwner returns (uint256 actualToAmount) {
        require(fromToken != address(0), "ZERO");
        require(toToken != address(0), "ZERO");
        require(fromToken != toToken, "SAME_ADDRESS");
        require(fromAmount > 0, "ZERO_FROM_AMOUNT");
