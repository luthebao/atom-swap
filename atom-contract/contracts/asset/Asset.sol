...
contract Asset is Ownable, ERC20 {
    ...
    event LiabilityAdded(
        uint256 previousLiabilityPosition,
        uint256 liabilityBeingAdded
    );

    /// @notice An event thats emitted when liability is removed
    event LiabilityRemoved(
        uint256 previousLiabilityPosition,
        uint256 liabilityBeingRemoved
    );
    ...
    modifier onlyFeeCollectorOrPool() {
        require(
            msg.sender == _feeCollector || msg.sender == _pool,
            "!permission"
        );
        _;
    }
    ...
    function transferUnderlyingToken(
        address to,
        uint256 amount
    ) external onlyFeeCollectorOrPool {
    ...
    function mint(address to, uint256 amount) external onlyPool {
        if (this.maxSupply() != 0) {
            ...
            require(
                amount + this.totalSupply() <= this.maxSupply(),
                "maxSupply"
            );
        }
