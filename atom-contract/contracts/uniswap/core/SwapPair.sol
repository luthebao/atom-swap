...
import "../lib/Reentrancy.sol";

// The base pair of pools, either stable or volatile
contract SwapPair is IERC20, ISwapPair, Reentrancy {
    ...
    constructor() {
        factory = msg.sender;
        treasury = ISwapFactory(msg.sender).treasury();
        (address _token0, address _token1) = ISwapFactory(msg.sender)
            .getInitializable();
    ...
    function metadata()
        external
        view
        returns (uint dec0, uint dec1, uint r0, uint r1, address t0, address t1)
    {
    ...
    function _update0(uint amount) internal {
        uint toTreasury = treasury == address(0)
            ? 0
            : (amount * ISwapFactory(factory).TREASURY_FEE()) /
                ISwapFactory(factory).DOMINATOR();
    ...
    function _update1(uint amount) internal {
        uint toTreasury = (amount * ISwapFactory(factory).TREASURY_FEE()) /
            ISwapFactory(factory).DOMINATOR();
        uint toFees = amount - toTreasury;
    ...
    function swap(
        ...
        require(!ISwapFactory(factory).isPaused(), "Pair: PAUSE");
        require(
        ...
        );
        {
            ...
            if (amount0In > 0) {
                _update0(
                    (amount0In * ISwapFactory(factory).SWAP_FEE()) /
                        ISwapFactory(factory).DOMINATOR()
                );
            }
            ...
            if (amount1In > 0) {
                _update1(
                    (amount1In * ISwapFactory(factory).SWAP_FEE()) /
                        ISwapFactory(factory).DOMINATOR()
                );
            }
    ...
    function getAmountOut(
        ...
        amountIn -=
            (amountIn * ISwapFactory(factory).SWAP_FEE()) /
            ISwapFactory(factory).DOMINATOR();
