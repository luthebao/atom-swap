...
contract SwapRouter is ISwapRouter {
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        SafeERC20.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWETH(WETH).deposit{value: amountETH}();
        assert(IWETH(WETH).transfer(pair, amountETH));
        liquidity = ISwapPair(pair).mint(to);
        // refund dust eth, if any
        if (msg.value > amountETH)
            SafeERC20.safeTransferETH(msg.sender, msg.value - amountETH);
    }
        ...
        returns (uint amountToken, uint amountETH)
    {
        ...
        SafeERC20.safeTransferETH(to, amountETH);
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        SafeERC20.safeTransferETH(to, amounts[amounts.length - 1]);
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            SafeERC20.safeTransferETH(msg.sender, msg.value - amounts[0]);
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        SafeERC20.safeTransferETH(to, amountOut);
    }
